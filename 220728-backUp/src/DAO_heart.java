...
//		String query = "delete from heart where id = ?";
	...
	public List<Integer> intliketop() throws SQLException{
		...
		String query = "select codiSet_id from (select codiSet_id ,count(*) as '숫자' from heart group by codiSet_id) as a order by '숫자' desc limit 3";
		Connection conn = null;
		PreparedStatement pstmt =null;
		ResultSet rs = null;
		try {
	...
	public List<Blob> imgliketop3() throws SQLException, MalformedURLException {
	      String query = "select codiset_image from (\r\n" + 
	      		"	select  codiSet_id from (\r\n" + 
	      		"	select codiSet_id ,count(*) as '숫자' from heart group by codiSet_id) as a\r\n" + 
	      		"	order by '숫자' desc limit 3\r\n" + 
	      		") as B Join codiset_new as C \r\n" + 
	      		"where B.codiSet_id = C.id;";
